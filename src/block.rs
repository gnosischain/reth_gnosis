...
use reth_evm::block::StateChangePreBlockSource;
use reth_primitives::Recovered;
use reth_provider::BlockExecutionResult;
use revm::context::Block;
use revm_database::State;
use reth_evm::{block::{BlockExecutor, BlockExecutorFactory, BlockExecutorFor, StateChangePostBlockSource, StateChangeSource, SystemCaller}, eth::{receipt_builder::{AlloyReceiptBuilder, ReceiptBuilder, ReceiptBuilderCtx}, spec::{EthExecutorSpec, EthSpec}, EthBlockExecutionCtx}, EvmFactory, FromRecoveredTx, OnStateHook};
use revm::{context::result::{ExecutionResult, ResultAndState}, DatabaseCommit, Inspector};
use revm_primitives::{Address, Log, B256};
    ...
    R: ReceiptBuilder,
{
    ...
    pub fn new(evm: Evm, ctx: EthBlockExecutionCtx<'a>, spec: Spec, receipt_builder: R, fee_collector_address: Address, block_rewards_address: Address) -> Self {
        Self {
            evm,
            ctx,
            receipts: Vec::new(),
    ...
    R: ReceiptBuilder<Transaction: Transaction + Encodable2718, Receipt: TxReceipt<Log = Log>>,
{
        ...
        if self.spec.is_prague_active_at_timestamp(self.evm.block().timestamp) {
            ...
            self.evm
                ...
                .increment_balances(HashMap::<_, _, revm_primitives::map::foldhash::fast::RandomState>::from_iter(vec![
                    (self.fee_collector_address, blob_gas * blob_gasprice),
                ]))
    ...
    fn finish(
        ...
        let requests = if self.spec.is_prague_active_at_timestamp(self.evm.block().timestamp) {
            ...
            let withdrawal_requests = self.system_caller.apply_withdrawal_requests_contract_call(&mut self.evm)?;
...
impl<R, Spec, EvmFactory> GnosisBlockExecutorFactory<R, Spec, EvmFactory> {
    ...
    /// [`ReceiptBuilder`].
    pub const fn new(receipt_builder: R, spec: Spec, evm_factory: EvmFactory, fee_collector_address: Address, block_rewards_address: Address) -> Self {
        Self { receipt_builder, spec, evm_factory, fee_collector_address, block_rewards_address }
    }
    ...
    Self: 'static,
{
    ...
    {
        GnosisBlockExecutor::new(evm, ctx, &self.spec, &self.receipt_builder, self.fee_collector_address, self.block_rewards_address)
    }
}
