...
if [ -d "$DB_PATH" ]; then
    ...
    STATE_ROOT=$(./target/debug/reth --chain "$SCRIPT_DIR/chainspecs/chiado.json" db --datadir "$DATA_DIR" get static-file headers 700000 | grep stateRoot | sed -E 's/.*: "(0x[0-9a-f]+)".*/\1/') || {
        STATE_ROOT=""
    }
    ...
    if [ "$STATE_ROOT" != "$EXPECTED_STATE_ROOT" ]; then
        ...
        ./target/debug/reth --chain $SCRIPT_DIR/chainspecs/chiado.json db --datadir "$DATA_DIR" drop -f || true
...
./target/debug/reth --chain "$SCRIPT_DIR/chainspecs/chiado.json" init-state $STATE_FILE --without-evm --header $HEADER_FILE --total-difficulty 231708131825107706987652208063906496124457284 --header-hash cdc424294195555e53949b6043339a49b049b48caa8d85bc7d5f5d12a85964b6 --datadir $DATA_DIR
